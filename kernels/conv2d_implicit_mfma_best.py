...
def compile_conv2d_implicit_mfma_best(n, c, h, width, k, r, s):
    p = h - r + 1
    q = width - s + 1
    npq = n * p * q
    crs = c * r * s
    ...
    allocator = SmemAllocator(None, arch=get_rocm_arch(), global_sym_name=f"conv_best_smem_{n}_{c}_{h}_{width}_{k}")
    ...
    def conv2d_implicit_mfma_best_kernel(y: fx.Tensor, x: fx.Tensor, w: fx.Tensor):
    ...
    @flyc.jit
    def launch_conv2d_implicit_mfma_best(y: fx.Tensor, x: fx.Tensor, w: fx.Tensor, stream: fx.Stream = fx.Stream(None)):
        allocator.finalized = False
        ctx = CompilationContext.get_current()
        with ir.InsertionPoint(ctx.gpu_module_body):
            allocator.finalize()

        conv2d_implicit_mfma_best_kernel(
            y,
            x,
            w,
        ).launch(
    ...
    return launch_conv2d_implicit_mfma_best


def conv2d_implicit_mfma_best_(y: torch.Tensor, x: torch.Tensor, w: torch.Tensor, stream=None):
    n, h, width, c = x.shape
    k, r, s, wc = w.shape
    assert c == wc
    exe = compile_conv2d_implicit_mfma_best(n, c, h, width, k, r, s)
