...
def compile_conv3d_implicit_8wave_fused_v6g(
    ...
    def conv3d_8wave_fused_v6g_kernel(y: fx.Tensor, x: fx.Tensor, weight: fx.Tensor, bias: fx.Tensor):
        ...
        # ---- prologue: tile 0 -> LDS through buffer_load_lds ----
        stage = 0
        next_stage = 1
        ...
        # ---- main loop: compute tile k, direct-load tile k+1 into next LDS ----
        if const_expr(tiles_per_split > 1):
            for kt_idx in range_constexpr(tiles_per_split - 1):
                acc00, acc01, a1_0, a1_1, b1_0 = compute_prefetch_phases(stage, a0_0, a0_1, b0_0)
                next_k = k_off + (kt_idx + 1) * TILE_K

                g2s_a_half(next_stage, 0, next_k)
                rocdl.sched_vmem(1)
                rocdl.s_setprio(1)
                acc10[0] = mfma_one(a1_0, b0_0, acc10[0])

                g2s_b_half(next_stage, 0, next_k)
                rocdl.sched_vmem(1)
                acc10[1] = mfma_one(a1_1, b0_0, acc10[1])

                g2s_a_half(next_stage, 1, next_k)
                rocdl.sched_vmem(1)
                acc11[0] = mfma_one(a1_0, b1_0, acc11[0])

                g2s_b_half(next_stage, 1, next_k)
                rocdl.sched_vmem(1)
    ...
    def launch(y: fx.Tensor, x: fx.Tensor, weight: fx.Tensor, bias: fx.Tensor, stream: fx.Stream = fx.Stream(None)):
        conv3d_8wave_fused_v6g_kernel(y, x, weight, bias).launch(
...
def conv3d_implicit_8wave_fused_v6g(x, weight, bias=None, stride=1, padding=0, splitk=None, stream=None):
    ...
    exe = compile_conv3d_implicit_8wave_fused_v6g(n, c, d, h, w, k, kt, kh, kw, st, sh, sw, pt, ph, pw, has_bias, sk)
