...
def compile_conv2d_implicit_mfma_pingpong(n, c, h, width, k, r, s):
    p = h - r + 1
    q = width - s + 1
    npq = n * p * q
    crs = c * r * s
    ...
    allocator = SmemAllocator(None, arch=get_rocm_arch(), global_sym_name=f"conv_pingpong_smem_{n}_{c}_{h}_{width}_{k}")
    ...
    def conv2d_implicit_mfma_pingpong_kernel(y: fx.Tensor, x: fx.Tensor, w: fx.Tensor):
        ...
        def compute_stage_part(stage, old_accs, wm_begin, wm_end):
            new_accs = [a for a in old_accs]
            sidx = fx.Index(stage)
            for kk in range_constexpr(WARP_K_STEPS):
                k_inner = kk * MFMA_K
                for wm in range_constexpr(wm_begin, wm_end):
        ...
        def compute_stage(stage, old_accs):
            return compute_stage_part(stage, old_accs, 0, WARP_M_STEPS)
        ...
        for kt in range_constexpr(k_tiles - (STAGES - 1)):
            ...
            rocdl.s_setprio(1)
            accs = compute_stage_part(stage, accs, 0, 1)
            rocdl.s_setprio(0)
            load_a_to_lds(next_k, write_stage)
            rocdl.s_setprio(1)
            accs = compute_stage_part(stage, accs, 1, WARP_M_STEPS)
            rocdl.s_setprio(0)
    ...
    @flyc.jit
    def launch_conv2d_implicit_mfma_pingpong(y: fx.Tensor, x: fx.Tensor, w: fx.Tensor, stream: fx.Stream = fx.Stream(None)):
        allocator.finalized = False
        ctx = CompilationContext.get_current()
        with ir.InsertionPoint(ctx.gpu_module_body):
            allocator.finalize()

        conv2d_implicit_mfma_pingpong_kernel(
            y,
            x,
            w,
        ).launch(
    ...
    return launch_conv2d_implicit_mfma_pingpong


def conv2d_implicit_mfma_pingpong_(y: torch.Tensor, x: torch.Tensor, w: torch.Tensor, stream=None):
    n, h, width, c = x.shape
    k, r, s, wc = w.shape
    assert c == wc
    exe = compile_conv2d_implicit_mfma_pingpong(n, c, h, width, k, r, s)
