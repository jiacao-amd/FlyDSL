...
# The ported kbuf1 main loop below is handwritten for this exact 8-wave shape.
...
def compile_conv3d_implicit_8wave_fused_v6h(
    ...
    def conv3d_8wave_fused_v6h_kernel(y: fx.Tensor, x: fx.Tensor, weight: fx.Tensor, bias: fx.Tensor):
        x_num_records = n * d * h * w * c
        w_num_records = k * crs
        ...
        x_buf = fx.rocdl.make_buffer_tensor(x, max_size=False, num_records_bytes=x_num_records * 2)
        w_buf = fx.rocdl.make_buffer_tensor(weight, max_size=False, num_records_bytes=w_num_records * 2)
        x_flat = fx.make_view(fx.get_iter(x_buf), fx.make_layout(x_num_records, 1))
        w_flat = fx.make_view(fx.get_iter(w_buf), fx.make_layout(w_num_records, 1))
        x_div = fx.logical_divide(x_flat, fx.make_layout(1, 1))
        w_div = fx.logical_divide(w_flat, fx.make_layout(1, 1))
        ...
        def copy_g2s(src_div, lds_array, elem_offset, src_elem):
            lds_byte_addr = fx.Int32(fx.ptrtoint(lds_array.ptr)) + fx.Int32(elem_offset * 2)
        ...
        def g2s_a_half(stage, m_half, k_base):
            ...
            in_h = oh * sh + kh_i - ph
            in_w = ow * sw + kw_i - pw
            valid_data = row_valid & in_range(in_t, d) & in_range(in_h, h) & in_range(in_w, w)
            g_elem = (((n_idx * d + in_t) * h + in_h) * w + in_w) * c + cc
            g_elem_i = arith.index_cast(T.i32, g_elem)
            safe_elem = arith.select(valid_data, g_elem_i, arith.constant(x_num_records, type=T.i32))
            half_base = fx.Index(stage) * TILE_M * TILE_K + m_half * HALF_M * TILE_K
            wave_base = half_base + wid * WARP_SIZE * LDG_VEC

            load_if = scf.IfOp(arith.unwrap(active), results_=[], has_else=False)
            with ir.InsertionPoint(load_if.then_block):
                copy_g2s(x_div, a_lds, wave_base, safe_elem)
                scf.YieldOp([])
        ...
        def g2s_b_half(stage, n_half, k_base):
            ...
            local_k = linear % TILE_K
            col = n_offset + fx.Index(n_half * HALF_N) + fx.Index(local_n)
            g_elem = col * crs + (fx.Index(k_base) + fx.Index(local_k))
            half_base = fx.Index(stage) * TILE_N * TILE_K + n_half * HALF_N * TILE_K
            wave_base = half_base + wid * WARP_SIZE * LDG_VEC

            if const_expr(n_tail):
                col_valid = col < fx.Index(k)
                g_elem_i = arith.index_cast(T.i32, g_elem)
                safe_elem = arith.select(col_valid, g_elem_i, arith.constant(w_num_records, type=T.i32))
                load_if = scf.IfOp(arith.unwrap(active), results_=[], has_else=False)
                with ir.InsertionPoint(load_if.then_block):
                    copy_g2s(w_div, b_lds, wave_base, safe_elem)
                    scf.YieldOp([])
            else:
                g_elem_i = arith.index_cast(T.i32, g_elem)
                load_if = scf.IfOp(arith.unwrap(active), results_=[], has_else=False)
                with ir.InsertionPoint(load_if.then_block):
                    copy_g2s(w_div, b_lds, wave_base, g_elem_i)
                    scf.YieldOp([])
        ...
        # ---- prologue: tile 0/1 -> LDS through buffer_load_lds ----
        stage = 0
        next_stage = 1
        g2s_full_tile(stage, k_off)
        if const_expr(tiles_per_split > 1):
            g2s_full_tile(next_stage, k_off + TILE_K)
        barrier(lgkmcnt=0)
        ...
        # ---- main loop: compute tile k; after reading current LDS, refill the
        # same LDS stage with tile k+2. That data is consumed two iterations
        # later from the same physical LDS addresses.
        for kt_idx in range_constexpr(tiles_per_split):
            acc00, acc01, a1_0, a1_1, b1_0 = compute_prefetch_phases(stage, a0_0, a0_1, b0_0)

            # Current stage is fully read now. The barrier makes that true for
            # all waves before this stage is overwritten, and also waits for the
            # previous iteration's k+2 prefetch before the next stage is read.
            if const_expr(kt_idx + 1 < tiles_per_split):
                barrier(lgkmcnt=0)

            if const_expr(kt_idx + 2 < tiles_per_split):
                prefetch_k = k_off + (kt_idx + 2) * TILE_K

                g2s_a_half(stage, 0, prefetch_k)
                rocdl.sched_vmem(1)
                rocdl.s_setprio(1)
                acc10[0] = mfma_one(a1_0, b0_0, acc10[0])

                g2s_b_half(stage, 0, prefetch_k)
                rocdl.sched_vmem(1)
                acc10[1] = mfma_one(a1_1, b0_0, acc10[1])

                g2s_a_half(stage, 1, prefetch_k)
                rocdl.sched_vmem(1)
                acc11[0] = mfma_one(a1_0, b1_0, acc11[0])

                g2s_b_half(stage, 1, prefetch_k)
                rocdl.sched_vmem(1)
            ...
            if const_expr(kt_idx + 1 < tiles_per_split):
                b0_0 = read_b_vec(next_stage, 0, 0)
                rocdl.sched_dsrd(1)
                stage = next_stage
    ...
    def launch(y: fx.Tensor, x: fx.Tensor, weight: fx.Tensor, bias: fx.Tensor, stream: fx.Stream = fx.Stream(None)):
        conv3d_8wave_fused_v6h_kernel(y, x, weight, bias).launch(
...
def conv3d_implicit_8wave_fused_v6h(x, weight, bias=None, stride=1, padding=0, splitk=None, stream=None):
    ...
    exe = compile_conv3d_implicit_8wave_fused_v6h(n, c, d, h, w, k, kt, kh, kw, st, sh, sw, pt, ph, pw, has_bias, sk)
